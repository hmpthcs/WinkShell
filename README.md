...
* lisgd
   ...
   * Note on scripts: these are very messy right now. Planning to replace most of the "toggle" scripts with separate on/show and off/hide scripts. Some started as toggles as I was using button presses to trigger these actions before. Scripts probably also should check the state of the apps they act on before doing anything, so that an erroneous swipe does not create a new instance of an already-running app (does the process exist? -> is the app visibile? -> do nothing).
